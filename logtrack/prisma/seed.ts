...
async function main() {
  ...
  try {
    ...
    await prisma.$transaction(async tx => {
      ...
      if (await tx.supervisor.count() || await tx.zona.count() || await tx.lote.count() || await tx.portal.count() || await tx.leitura.count()) {
      ...
      await tx.portal.createMany({ data: state.portais });
    ...
    console.log("Criados: 1 supervisor, 3 zonas, 3 portais e 5 lotes.");
  } finally { await prisma.$disconnect(); }
  const activations = JSON.parse(await readFile(path.join(folder, "ativacoes.json"), "utf8")) as { nome: string; path: string }[];
  const guide = "# Ativação dos portais\n\nAbra cada URL somente no aparelho correspondente. Se um token for regenerado no painel, use o novo link.\n\n" + activations.map(a => "- " + a.nome + ": " + origin + a.path).join("\n") + "\n";
