...
export function StationForm({ zones, station }: { zones: Zona[]; station?: DashboardData["portais"][number] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError(""); const f = new FormData(event.currentTarget);
    try {
      const result = await api<{ activationUrl?: string }>("portais" + (station ? "/" + station.id : ""), station ? "PATCH" : "POST", { nome: f.get("nome"), ...(!station ? { zonaId: f.get("zonaId") } : {}) });
      if (result.activationUrl) setActivation(result.activationUrl); else router.push("/portais"); router.refresh();
    } catch (e) { setError((e as Error).message); } finally { setBusy(false); }
  }
  return <><Back href="/portais">Voltar para portais</Back><PageTitle title={station ? "Editar portal" : "Cadastrar portal"} description="Vincule um portal a uma zona do galpão."/>{activation ? <section className="panel management-form"><CheckCircle2 className="success-icon" size={40}/><h2>Portal cadastrado</h2><p>Abra este link no Chrome do aparelho que será a estação. Guarde-o para uma futura reativação.</p><CopyUrl url={activation}/><div className="info-box">Depois da ativação, adicione a página da estação à tela inicial do aparelho.</div><Link className="button primary" href="/portais">Ver portais</Link></section> : <form className="panel management-form" onSubmit={submit}><label>Nome do portal<input name="nome" required maxLength={100} defaultValue={station?.nome} placeholder="Ex.: Portal 4 — Qualidade"/></label><label>Zona vinculada<select name="zonaId" required defaultValue={station?.zonaId || ""} disabled={Boolean(station)}><option value="" disabled>Selecione uma zona</option>{zones.filter(z => z.ativa || z.id === station?.zonaId).map(z => <option key={z.id} value={z.id}>{z.nome}</option>)}</select></label><div className="info-box"><Smartphone size={21}/><span>A zona de uma estação é fixa. Para mudar de área, desative esta estação e cadastre outra.</span></div>{!zones.some(z => z.ativa) && !station && <p>Cadastre uma <Link className="text-link" href="/zonas/nova">zona ativa</Link> primeiro.</p>}{error && <div className="form-error" role="alert">{error}</div>}<div className="form-actions"><Link className="button secondary" href="/portais">Cancelar</Link><button className="button primary" disabled={busy || (!station && !zones.some(z => z.ativa))}>{busy ? "Salvando..." : station ? "Salvar alterações" : "Cadastrar e gerar link"}</button></div></form>}</>;
}
...
export function TagWriter({ lot }: { lot: Lote }) {
  ...
  async function write() {
    ...
    if (!NDEFReader || !window.isSecureContext) { setError("Use Chrome no Android com RFID e acesse por HTTPS."); return; }
    if (!url.startsWith("https://")) { setError("Abra o sistema pelo endereço HTTPS antes de gravar a etiqueta para uso no aparelho."); return; }
    ...
    catch (e) { setPhase("idle"); if ((e as Error).name !== "AbortError") setError("Falha ao gravar. Verifique a permissão, o RFID e a etiqueta, e tente novamente."); }
  }
  return <><Back/><PageTitle title="Gravar etiqueta" description={lot.codigo + " · " + (lot.descricao || "Lote cadastrado")}/><section className="panel nfc-writer"><div className="nfc-write-icon"><Radio size={45}/></div><h2>{phase === "done" ? "Etiqueta gravada" : "Seu lote, a uma aproximação de distância."}</h2><p>Grave o endereço abaixo na etiqueta RFID e cole-a no lote.</p><CopyUrl url={url}/>{!supported && <div className="info-box">A gravação RFID não está disponível neste navegador. Abra esta mesma tela no Chrome de um Android com RFID, pelo endereço HTTPS do sistema.</div>}{error && <div className="form-error" role="alert">{error}</div>}{phase === "done" ? <div role="status"><CheckCircle2 className="success-icon" size={34}/><p>Etiqueta gravada com sucesso. O lote está pronto para leitura.</p><Link href="/lotes" className="button primary">Voltar para lotes</Link></div> : phase === "written" ? <button className="button primary" onClick={markWritten}>Salvar confirmação novamente</button> : <button className="button primary" disabled={!supported || phase !== "idle" || lot.arquivado} onClick={write}><Radio size={18}/>{phase === "saving" ? "Salvando..." : "Gravar etiqueta"}</button>}<small>A leitura da etiqueta por um portal ativado registra a movimentação automaticamente.</small></section><dialog ref={dialog} className="nfc-dialog" aria-labelledby="nfc-dialog-title" onCancel={() => controller.current?.abort()}><Radio size={48} className="pulse"/><h2 id="nfc-dialog-title">Aproxime uma etiqueta RFID virgem do aparelho</h2><p>Mantenha a etiqueta próxima até a confirmação.</p><button className="button secondary" onClick={() => controller.current?.abort()}>Cancelar gravação</button></dialog></>;
}
