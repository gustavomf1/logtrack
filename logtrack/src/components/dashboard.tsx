...
import { ArrowDownLeft, ArrowRight, ArrowUpRight, CheckCircle2, Clock3, MapPin, Package, Plus, Radio, Search, Smartphone, Tag, TriangleAlert } from "lucide-react";
...
export function Dashboard({ initialData: data, mapaData, path, authenticated = true }: { initialData: DashboardData; mapaData?: MapaData | null; path: string[]; authenticated?: boolean }) {
  ...
  if (!section) content = <Overview data={data} authenticated={authenticated}/>;
    ...
    else content = <><PageTitle title="Zonas" description="Cada espaço, conectado à sua operação."><Link className="button primary" href="/zonas/nova"><Plus size={17}/>Nova zona</Link></PageTitle><div className="zone-grid">{data.zonas.map((zone, i) => <article className="panel zone-card" key={zone.id}><div className="card-top"><span className={"zone-icon tone-" + i % 3}><MapPin size={23}/></span><span className={"badge " + (zone.ativa ? "green" : "gray")}>{zone.ativa ? "Ativa" : "Inativa"}</span></div><h2>{zone.nome}</h2><p>{zone.descricao || "Sem descrição"}</p><div className="zone-counts"><Link href={"/lotes?zona=" + zone.id}><strong>{data.lotes.filter(l => !l.arquivado && l.zonaAtualId === zone.id).length}</strong> lotes</Link><span><strong>{data.portais.filter(c => c.zonaId === zone.id && c.ativo).length}</strong> estações ativas</span></div><div className="card-actions"><Link className="text-link" href={"/zonas/" + zone.id}>Editar zona <ArrowUpRight size={15}/></Link><button className="text-button" disabled={busy} onClick={() => change("zonas/" + zone.id, "PATCH", { ativa: !zone.ativa })}>{zone.ativa ? "Desativar" : "Reativar"}</button></div></article>)}</div>{!data.zonas.length && <Empty>Cadastre a primeira zona do galpão.</Empty>}</>;
  } else if (section === "portais") {
    const station = data.portais.find(c => c.id === id);
    if (id === "novo" || station) content = <StationForm zones={data.zonas} station={station}/>;
    else content = <><PageTitle title="Portais" description="Uma estação para cada zona. Movimentações sem digitação."><Link className="button primary" href="/portais/novo"><Plus size={17}/>Cadastrar portal</Link></PageTitle>{activation && <div className="info-box activation-box" role="status"><strong>Novo link de ativação</strong><p>O vínculo anterior foi invalidado. Abra este link no aparelho da estação.</p><CopyUrl url={activation}/></div>}<div className="panel table-scroll"><table><thead><tr><th>Estação</th><th>Zona vinculada</th><th>Última leitura</th><th>Status</th><th>Ações</th></tr></thead><tbody>{data.portais.map(c => <tr key={c.id}><td><div className="cell-title"><Smartphone size={18}/><Link href={"/portais/" + c.id}>{c.nome}</Link></div></td><td>{zoneName(data.zonas, c.zonaId)}</td><td>{datetime(c.ultimoUso)}</td><td><span className={"badge " + (c.ativo ? "green" : "gray")}>{c.ativo ? "Ativa" : "Inativa"}</span></td><td><div className="table-actions"><button className="text-button" disabled={busy} onClick={() => change("portais/" + c.id + "/regenerar-token", "POST", {})}>Regenerar token</button><button className="text-button" disabled={busy} onClick={() => change("portais/" + c.id, "PATCH", { ativo: !c.ativo })}>{c.ativo ? "Desativar" : "Reativar"}</button></div></td></tr>)}</tbody></table>{!data.portais.length && <Empty>Cadastre um portal e vincule-o a uma zona.</Empty>}</div><div className="info-box"><Radio size={20}/><span>Abra o link de ativação no aparelho físico. Depois, basta aproximar uma etiqueta para registrar a movimentação.</span></div></>;
  }
  return <>{error && <div className="form-error" role="alert">{error}</div>}{content}</>;
}

function Overview({ data, authenticated }: { data: DashboardData; authenticated: boolean }) {
  ...
  const activeStations = data.portais.filter(c => c.ativo).length;
  const metrics = [
    { label: "No estoque", value: lots.length, note: "lotes ativos", tone: "blue" },
    { label: "Localizados", value: located.length, note: "em alguma zona", tone: "green" },
    { label: "Sem zona", value: lots.length - located.length, note: "aguardando leitura", tone: "purple" },
    { label: "Atenção", value: attention.length, note: "vencidos ou a vencer", tone: "amber" },
  ];
  return <><PageTitle title="Visão geral" description="Acompanhe seus lotes. Mantenha tudo no lugar certo.">{authenticated && <><Link className="button secondary" href="/lotes">Ver todos os lotes <ArrowUpRight size={16}/></Link><Link className="button primary" href="/lotes/novo"><Plus size={18}/>Novo lote</Link></>}</PageTitle>
    <section className="overview-banner"><div><span className="banner-kicker"><span className="live-dot"/>{activeStations} {activeStations === 1 ? "estação conectada" : "estações conectadas"}</span><h2>Do físico ao digital.<br/>Sem perder o caminho.</h2><p>Cada aproximação conecta um lote à sua localização. Acompanhe o estoque em tempo real.</p><Link href="/ajuda">Veja como funciona</Link></div></section>
    <div className="ops-bar">{metrics.map(({ label, value, note, tone }) => <div className={"ops-metric " + tone} key={label}><strong>{value.toLocaleString("pt-BR")}</strong><span>{label}</span><small>{note}</small></div>)}</div>
    <div className="overview-columns"><section className="panel"><div className="panel-heading"><div><h2>Distribuição por zona</h2><p>Onde seus lotes estão agora</p></div><Link href="/zonas" className="text-link">Ver zonas <ArrowUpRight size={15}/></Link></div><div className="distribution">{[...data.zonas.filter(z => z.ativa).map(z => ({ id: z.id as string | null, nome: z.nome })), { id: null, nome: "Sem Zona" }].map((z, i) => { const count = lots.filter(l => l.zonaAtualId === z.id).length; return <Link href={"/lotes?zona=" + (z.id || "sem-zona")} className="distribution-row" key={z.id || "none"}><span className={"distribution-mark tone-" + i % 4}><MapPin size={17}/></span><div><div className="distribution-label"><strong>{z.nome}</strong><span>{count} <small>lotes</small></span></div><div className="bar-track"><div className={"bar-fill tone-" + i % 4} style={{ width: (lots.length ? count / lots.length * 100 : 0) + "%" }}/></div></div></Link>; })}</div><div className="panel-foot"><span className="live-dot"/>{data.portais.filter(c => c.ativo).length} estações ativas na operação</div></section>
    <section className="panel"><div className="panel-heading"><div><h2>Movimentações recentes</h2><p>Os últimos passos da sua operação</p></div><span className="soft-icon"><ArrowDownLeft size={20}/></span></div><div className="activity-list">{data.movimentacoes.slice(0, 4).map(m => <Link className="activity-item" href={"/lotes/" + m.loteId} key={m.id}><span className={"activity-icon " + (m.tipo === "CANCELAMENTO" ? "amber" : "green")}><ArrowRight size={17}/></span><div><strong>{data.lotes.find(l => l.id === m.loteId)?.codigo}</strong><p>{m.tipo === "CANCELAMENTO" ? "Cancelado · Sem Zona" : zoneName(data.zonas, m.zonaDestinoId)}</p><small>{data.portais.find(c => c.id === m.portalId)?.nome}</small></div><time>{datetime(m.timestamp)}</time></Link>)}{!data.movimentacoes.length && <Empty>As movimentações aparecerão aqui após a primeira leitura RFID.</Empty>}</div></section></div>
    <section className="panel"><div className="panel-heading"><div><h2>Lotes no estoque <span className="count-pill">{lots.length}</span></h2><p>Rastreabilidade em cada item, do início ao fim</p></div><Link href="/lotes" className="text-link">Ver todos <ArrowRight size={16}/></Link></div><LotTable lots={lots.slice(0, 5)} data={data} readOnly={!authenticated}/></section>
    <LiveNotifications data={data}/>
  </>;
}
...
function Help() {
  return <><PageTitle title="Um toque. Tudo localizado." description="Conheça o fluxo de rastreamento do LogTrack."/><div className="help-grid">{[{ n: "01", title: "Organize as zonas", text: "Cadastre as áreas do galpão e um portal para cada estação. Abra o link de ativação no Chrome do aparelho correspondente.", href: "/zonas", label: "Gerenciar zonas" }, { n: "02", title: "Identifique os lotes", text: "Cadastre um lote e grave a URL em uma etiqueta RFID pelo aparelho do supervisor. Cole a etiqueta no lote.", href: "/lotes/novo", label: "Cadastrar lote" }, { n: "03", title: "Aproxime para movimentar", text: "Com o portal ativado, aproxime a etiqueta. A página abre e registra a zona automaticamente. Aguarde a confirmação na tela.", href: "/portais", label: "Ver estações" }].map(step => <article className="panel help-step" key={step.n}><span>{step.n}</span><h2>{step.title}</h2><p>{step.text}</p><Link className="text-link" href={step.href}>{step.label}<ArrowRight size={16}/></Link></article>)}</div><section className="panel help-rules"><h2>Como a movimentação funciona</h2><p><strong>Lote Sem Zona:</strong> vai para a zona do portal que fez a leitura.</p><p><strong>Nova leitura pelo mesmo portal:</strong> cancela a movimentação e volta para Sem Zona.</p><p><strong>Leitura por outro portal:</strong> vai para a zona desse outro portal, mesmo que ambos pertençam à mesma zona.</p><p><strong>Portal sem vínculo ativo:</strong> abre somente para consulta.</p><div className="info-box"><TriangleAlert size={21}/><span>Cada nova abertura ou atualização da página do lote em um portal ativado conta como uma leitura. Aguarde o resultado antes de aproximar novamente.</span></div><h2>Para gravar etiquetas</h2><p>Use Chrome no Android com RFID ativado e acesse o sistema por HTTPS. A gravação exige permissão do navegador. No computador, copie a URL e abra a tela de gravação no aparelho do supervisor.</p><p>Se um portal for perdido ou substituído, desative a estação ou regenere seu token na tela de portais.</p></section></>;
}
