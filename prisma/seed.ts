...
async function main() {
  ...
  if (!email || !password || password.length < 10) throw new Error("Configure SUPERVISOR_EMAIL e SUPERVISOR_PASSWORD (mínimo 10 caracteres).");
  ...
  if (process.env.DEMO_MODE === "true" && !process.env.VERCEL) {
    ...
    await readState();
    console.log("Dados locais prontos. Registros existentes preservados.");
  } else {
